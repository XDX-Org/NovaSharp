...
{
    private sealed record CachedCompletion(RoslynLanguageSnapshot Snapshot, CompletionItem Item);
    ...
    public LanguageServiceMetrics Metrics => new(
        _work.TotalCapacity,
    ...
    public Task<LanguageCompletionList?> GetCompletionsAsync(LanguageRequest request, CancellationToken cancellationToken = default) =>
        RunLatestAsync(request, "completion", async (snapshot, document, token) =>
        {
            ...
            var list = await service.GetCompletionsAsync(
                document,
                ClampPosition(request.Position, await document.GetTextAsync(token).ConfigureAwait(false)),
                trigger: trigger,
                cancellationToken: token).ConfigureAwait(false);
            if (list is null) return new LanguageCompletionList(request.RequestId, request.SourceVersion, request.Sequence, false, []);

            var items = list.ItemsList.Take(MaximumCompletionItems).Select(item =>
            {
                var id = $"{request.RequestId}:{Guid.NewGuid():N}";
                CacheCompletion(id, new(snapshot, item));
            ...
            return new LanguageCompletionList(
                ...
                request.Sequence,
                list.ItemsList.Count > MaximumCompletionItems,
                items);
        }, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            if (snapshot.DocumentId != cached.Snapshot.DocumentId
                || snapshot.ProjectId != cached.Snapshot.ProjectId
                || snapshot.SourceVersion != cached.Snapshot.SourceVersion
                || snapshot.Sequence != cached.Snapshot.Sequence
                || snapshot.ReplicaVersion != cached.Snapshot.ReplicaVersion)
            {
                return null;
            }
            var service = CompletionService.GetService(document);
    ...
    private void CacheCompletion(string id, CachedCompletion completion)
    {
        lock (_gate)
        {
    ...
    {
        if (tags.Contains(WellKnownTags.Method)) return "method";
