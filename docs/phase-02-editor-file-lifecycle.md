...
|---|---|---|
| Cold process start to an interactive editor | macOS Arm64 3,500 ms; macOS x64 4,000 ms; all other fixtures 2,500 ms | `src/NovaSharp/Workbench.cs`, fresh process after disposable browser-profile provisioning |
| Warm process start to an interactive editor | Linux 1,600 ms; macOS Arm64 3,500 ms; macOS x64 4,000 ms; Windows 1,600 ms | Median of three fresh processes sharing that profile |
...
The retained native and browser records include the fixture-specific limits used for their gates. The macOS startup
fixtures include the Phase 3 workbench and are calibrated from their retained attempts on `macos-15` and
`macos-15-intel`.
